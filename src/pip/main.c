...
#define TASK3_PERIOD 	7000

/* Tasks WCETs. */
#define TASK1_WCET		1000
#define TASK2_WCET		1500
#define TASK3_WCET		2500
 ...
 * Periodic task.
 */
static void prvTareaAlta( void* pvParameters );
static void prvTareaMedia( void* pvParameters );
static void prvTareaBaja( void* pvParameters );
...
int main( void )
{
    ...
    xTaskCreate( prvTareaAlta, "Tarea 1", configMINIMAL_STACK_SIZE + 50, (void*) &task1, configMAX_PRIORITIES - 1, NULL );
    xTaskCreate( prvTareaMedia, "Tarea 2", configMINIMAL_STACK_SIZE + 50, (void*) &task2, configMAX_PRIORITIES - 2, NULL );
    xTaskCreate( prvTareaBaja, "Tarea 3", configMINIMAL_STACK_SIZE + 50, (void*) &task3, configMAX_PRIORITIES - 3, NULL );
...
void prvTareaBaja( void *pvParameters )
{
	...
	for( ;; )
	{
        if (xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE) {
            prvPrintString("Tarea Baja: tengo el semáforo\n\r");
            ...
            prvPrintString("Tarea Baja: liberé el semáforo\n\r");
...
void prvTareaMedia( void *pvParameters )
{
	char cMessage[ mainMAX_MSG_LEN ];
	unsigned int uxReleaseCount = 0;
	TickType_t pxPreviousWakeTime = 0;
...
void prvTareaAlta( void *pvParameters )
{
	char cMessage[ mainMAX_MSG_LEN ];
	unsigned int uxReleaseCount = 0;
	TickType_t pxPreviousWakeTime = 0;
	...
	{
        if (xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE) {
            prvPrintString("Tarea Alta: tengo el semáforo\n\r");
            sprintf( cMessage, "S: %s - %u - %u\n\r", pcTaskGetTaskName( NULL ), uxReleaseCount, xTaskGetTickCount());
	        ...
	        xSemaphoreGive(xSemaphore);
            prvPrintString("Tarea Alta: liberé el semáforo\n\r");
...

{
    ...
    if (xSemaphore == NULL) {
        ...
        /* Se libera el semáforo para comenzar */
        xSemaphoreGive(xSemaphore);
    }
